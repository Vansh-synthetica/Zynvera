...
export default function ParentLinkPage() {
  ...
  return (
    <AppShell>
      <div className="mx-auto max-w-xl px-4 sm:px-6 py-6 space-y-6">
        <Link href="/parent/dashboard" className="inline-flex items-center gap-2 text-sm text-muted-foreground hover:text-foreground">
          <span className="text-lg leading-none">&larr;</span> Family Dashboard
        </Link>

        <div>
          <h1 className="text-2xl font-semibold tracking-tight">Link Your Child</h1>
          <p className="text-sm text-muted-foreground mt-0.5">
            Ask your child for the Family Code on their profile, then request access.
          </p>
        </div>

        <div className="neo rounded-2xl p-5 space-y-4">
          {error && (
            <div className="flex items-start gap-2 rounded-xl bg-destructive/5 border border-destructive/20 p-3 text-xs text-destructive">
          ...
          <form onSubmit={handleSubmit} className="space-y-3">
            ...
            <div className="space-y-1.5">
              ...
              <p className="text-[11px] text-muted-foreground">
                Found in their app under Family Code.
              </p>
            </div>
        ...
        {/* Existing requests */}
        <div>
          <h2 className="text-sm font-semibold mb-3">Your requests</h2>
          {loadingReqs ? (
            <div className="flex items-center justify-center py-8 text-muted-foreground">
              <Loader2 className="size-4 animate-spin mr-2" /> Loading...
            </div>
          ) : requests.length === 0 ? (
            <div className="neo rounded-2xl py-8 text-center">
              <p className="text-sm text-muted-foreground">No requests yet.</p>
            </div>
          ) : (
            <div className="space-y-2">
              {requests.map(r => (
                <div key={r.id} className="neo-sm rounded-2xl p-3.5 flex flex-wrap items-center gap-2 text-sm">
                  ...
                  <span className="ml-auto shrink-0">
                    {r.status === 'approved' && (
                      <Badge variant="success" className="gap-1"><CheckCircle2 className="size-3" /> Approved</Badge>
                    )}
                    {r.status === 'pending' && (
                      <Badge variant="secondary" className="gap-1"><Clock className="size-3" /> Pending</Badge>
                    )}
                    {r.status === 'rejected' && (
