...
export default function TeacherMessagesPage() {
    ...
    const channel = supabase
      ...
      .on(
        ...
        payload => {
          ...
          if (
            activeIdRef.current &&
            (row.sender_id === activeIdRef.current || row.recipient_id === activeIdRef.current) &&
  ...
  const openThread = useCallback(
    async (otherId: string) => {
      ...
      try {
        ...
        setThread(rows ?? [])
        await markAsRead(otherId, userId!) // mark their messages read
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      setDraft('')
      // Optimistic append keeps the thread snappy.
      setThread(prev => [
        ...prev,
        {
          id: `tmp-${Date.now()}`,
  ...
  const nameOf = (conv?: Conversation | null) =>
      ...
      <div className="mx-auto max-w-6xl px-4 py-6 space-y-4">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            ...
            <p className="text-sm text-muted-foreground">Direct messaging with students</p>
          </div>
            ...
            {activeId ? (
              <>
                <div className="border-b px-4 py-3 flex items-center gap-2">
                  ...
                  <p className="font-medium text-sm truncate">{nameOf(activeConv)}</p>
              ...
              <CardContent className="flex-1 flex items-center justify-center py-24 text-center">
                <div className="space-y-2">
                  ...
                  <p className="text-xs text-muted-foreground">
                    Or start a new one — students can also message you first.
                  </p>
                </div>
              </CardContent>
            )}
      ...
      <Dialog open={newOpen} onOpenChange={setNewOpen}>
        <DialogContent className="max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>Find a student or colleague in your institution.</DialogDescription>
          </DialogHeader>
              ...
              <div className="rounded-md border divide-y max-h-[240px] overflow-y-auto">
                {results.map(u => (
                  ...
                  >
                    ...
                    <Badge variant="outline" className="ml-auto shrink-0 capitalize">{u.role}</Badge>
                  </button>
                ))}
              </div>
            )}
